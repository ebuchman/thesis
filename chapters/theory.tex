...
Note the pi calculus doesn't allow a uniform encoding of broadcast \cite{ene1999expressiveness},
but we don't need uniformity, since in practice each node has a network stack/kernel that manages broadcasts.
...
Informally, reliable broadcast (RBC) is a broadcast primitive satisfying
...
Formally, we model RBC as a pi-calculus process, 
...
\ifx
We can state the properties in a temporal henessy-milner logic:
\begin{itemize}
\item validity - $ rbc_i |= [ r_i?(v) ] eventually < d_i!(v) > $
\item agreement - $ rbc_i |= if a correct process delivers m, all correct processes eventually deliver m $
\item integrity - m is only delivered once, and only if broadcast by its sender
\end{itemize}
\fi
...
Bitcoin makes synchronicity assumption that network latency is much less than block time.
Is the asynchronous generalization of bitcoin casper?
