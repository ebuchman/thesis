...
On the other-hand, the need for synchrony can be abolished by using non-determinism in a way that guarantees consensus, eventually, with probability 1 \ref{}.
...
In fact, it can be shown that the upper limit on $f$ for Byzantine faults is $f < N/3$ \ref{pease1980reaching}.
...
The first generally accepted solution was the DLS algorithm, named after it's inventors \ref{dls}, and described in \ref{fig:dls}.
